...
#define WORM_COUNT 1                 // Number of agents
...
__global__ void moveAgents(Agent* agents, curandState* states, float* grid) {
    int id = threadIdx.x + blockIdx.x * blockDim.x;
    if (id < WORM_COUNT) {
        //float dx = (curand_uniform(&states[id]) - 0.5f) * 2.0f;  // Move in x direction
        //float dy = (curand_uniform(&states[id]) - 0.5f) * 2.0f;  // Move in y direction
        //float dx = (target_x - agents[id].x) * SPEED;
        //float dy = (target_y - agents[id].y) * SPEED;
        ...
        for (int i = -SENSING_RANGE; i <= SENSING_RANGE; ++i) {
            for (int j = -SENSING_RANGE; j <= SENSING_RANGE; ++j) {
                ...
                //if(id==0) {
                 //   printf("Agent %d sensed concentration %f at (%d, %d)\n", id, concentration, xIndex, yIndex);
                //}
        ...
        //float new_angle = LAMBDA * agents[id].angle + (1.0f - LAMBDA) * (curand_uniform(&states[id]) * 2.0f * M_PI + DRIFT_FACTOR * max_concentration*bias);
        float sensed_odor = grid[agent_x * N + agent_y];
        float new_speed = SPEED;
        if(sensed_odor>0.0f){// || sensed_odor<ODOR_THRESHOLD){
            new_speed = ATTRACTION_STRENGTH * log10(ATTRACTION_SCALE + sensed_odor);
        }
...
__global__ void updateGrid(float* grid) {
    ...
    if (i < N && j < N) {
        ...
        if (new_concentration < 0) new_concentration = 0;
...
// Check for errors in the kernel launch
    ...
    //initGrid<<<(N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(grid);
    cudaDeviceSynchronize();
    ...
    for (int i = 0; i < N_STEPS; ++i) {
        ...
        moveAgents<<<(WORM_COUNT + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(d_agents, d_states, grid);
        // Check for errors in the kernel launch
        cudaError_t err = cudaGetLastError();
        if (err != cudaSuccess) {
            printf("CUDA error: %s\n", cudaGetErrorString(err));
