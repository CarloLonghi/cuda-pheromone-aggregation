...
weights = torch.tensor([1, 0.009, 0.004, 0.001, 0.0, 0.0, 0.0, 0.0])
...
plt.savefig(f"./results/plots/{weights[0].item():.4f}_{weights[1].item():.4f}_{weights[2].item():.4f}_{weights[3].item():.4f}_{weights[4].item():.4f}_{weights[5].item():.4f}_{weights[6].item():.4f}_{weights[7].item():.4f}.png")
plt.show()